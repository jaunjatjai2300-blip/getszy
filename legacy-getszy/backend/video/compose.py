...
async def build_video(scenes: List[Dict[str, Any]], audio_path: str, out_path: str,
    ...
    if not clip_paths:
        return {'error': f'no scenes composed — all {len(scenes)} scene clips failed to encode via ffmpeg. Check ffmpeg binary + image validity.'}
    # 2. Concat clips
    ...
    if proc.returncode != 0:
        ...
        try:
            ...
            print(f'[compose] mux failed, saved silent video. err={(err or b"")[:300]!r}')
            return {'path': out_path, 'scenes': len(clip_paths), 'warning': 'audio_mux_failed_silent_video'}
        except Exception:
            return {'error': f'mux failed: {(err or b"")[:300].decode("utf-8", "ignore")}'}
    # Cleanup tmp
    for cp in clip_paths:
        try: os.remove(cp)
        except Exception: pass
