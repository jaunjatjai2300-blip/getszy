...
async def generate_code_file(
    ...
    try:
        code = await chat_completion(system=system, user=user, temperature=0.25, max_tokens=8192, session_id=f'saas-code-{filename}')
    except Exception:
        code = ''
    ...
    for _ in range(2):
        ...
        try:
            fixed = await chat_completion(system=fix_system, user=fix_user, temperature=0.1, max_tokens=8192, session_id=f'saas-review-{filename}')
...
async def get_file_query(project_id: str, path: str = '', admin=Depends(get_current_admin)):
