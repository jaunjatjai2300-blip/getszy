...
import { LayoutDashboard, Package, ShoppingCart, Truck, Users, Sparkles, LogOut, Store, GraduationCap, Activity, TrendingUp, Rocket, Wand2, Layers, MessageCircle, PenTool, Film, Share2, Briefcase, ChevronDown, ChevronRight } from "lucide-react";
    ...
    <div className="min-h-screen grid md:grid-cols-[240px_1fr]" style={{ background: "#F7F5F2" }}>
      <aside className="hidden md:flex flex-col border-r" style={{ background: "var(--gs-surface)", borderColor: "var(--gs-border)" }}>
              ...
              <div key={g.label}>
                ...
                {isOpen && (
                  <div className="ml-3 space-y-0.5 border-l" style={{ borderColor: "var(--gs-border)" }}>
                    {g.items.map((n) => (
                      <NavLink key={n.to} to={n.to} data-testid={`admin-nav-${n.label.toLowerCase().replace(/\s+/g, "-")}`}
        ...
        <div className="p-3 border-t" style={{ borderColor: "var(--gs-border)" }}>
          <button onClick={() => navigate("/")} className="w-full flex items-center gap-2 px-3 py-2 text-xs text-[var(--gs-muted)] hover:text-[var(--gs-ink)]" data-testid="admin-view-store-button"><Store className="h-4 w-4"/>View Storefront</button>
          <button onClick={() => { logout(); navigate("/"); }} className="w-full flex items-center gap-2 px-3 py-2 text-xs text-[var(--gs-muted)] hover:text-[var(--gs-ink)]" data-testid="admin-logout-button"><LogOut className="h-4 w-4"/>Logout</button>
        </div>
      </aside>
      <div className="md:hidden flex items-center justify-between p-4 border-b" style={{ background: "var(--gs-surface)", borderColor: "var(--gs-border)" }}>
        ...
        <NavLink to="/admin" className="text-xs px-3 py-1.5 rounded-lg bg-[var(--gs-teal)] text-white flex items-center gap-1"><Sparkles className="h-3 w-3"/>Neo</NavLink>
      </div>
