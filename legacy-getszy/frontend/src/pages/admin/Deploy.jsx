...
export default function AdminDeploy() {
  ...
  const loadCommits = async () => {
    try {
      const r = await api.get("/admin/deploy/commits?limit=8");
      setCommits(r.data.items || []);
      if (!r.data.ok) setCommitsMsg(r.data.message || "");
    } catch (e) {}
  };
  useEffect(() => { loadStatus(); loadJobs(); loadCommits(); }, []);
  ...
  return (
    <div className="space-y-6" data-testid="admin-deploy-page">
      ...
      {/* Recent GitHub commits (rollback context) */}
      <div>
        <div className="flex items-center gap-2 mb-3">
          <h3 className="font-display text-xl flex items-center gap-2"><GitCommit className="h-4 w-4 text-[var(--gs-teal)]"/>Recent commits</h3>
          {status?.github?.repo && status.github.configured && (
            <a href={`https://github.com/${status.github.repo}/commits`} target="_blank" rel="noreferrer" className="text-xs text-[var(--gs-teal)] hover:underline flex items-center gap-1" data-testid="commits-github-link">
              <ExternalLink className="h-3 w-3"/>{status.github.repo}
            </a>
          )}
          <button onClick={loadCommits} className="ml-auto text-xs text-[var(--gs-muted)] hover:text-[var(--gs-teal)] flex items-center gap-1" data-testid="commits-refresh">
            <RefreshCw className="h-3 w-3"/>Refresh
          </button>
        </div>
        {commits.length === 0 ? (
          <div className="gs-card p-6 text-center text-xs text-[var(--gs-muted)]" data-testid="commits-empty">
            {commitsMsg || "GitHub commits will appear here once GITHUB_TOKEN + GITHUB_REPO are configured."}
          </div>
        ) : (
          <div className="gs-card divide-y" style={{ borderColor: "var(--gs-border)" }} data-testid="commits-list">
            {commits.map(c => (
              <div key={c.sha_full} className="p-3 flex items-start gap-3" data-testid={`commit-${c.sha}`}>
                <code className="text-[10px] px-1.5 py-0.5 rounded bg-[var(--gs-surface-2)] font-mono shrink-0 mt-0.5">{c.sha}</code>
                <div className="flex-1 min-w-0">
                  <div className="text-sm line-clamp-1">{c.message}</div>
                  <div className="text-[11px] text-[var(--gs-muted)] mt-0.5">
                    {c.author} · {c.date ? new Date(c.date).toLocaleString() : ""}
                  </div>
                </div>
                {c.html_url && (
                  <a href={c.html_url} target="_blank" rel="noreferrer" className="text-[var(--gs-teal)]" title="View on GitHub">
