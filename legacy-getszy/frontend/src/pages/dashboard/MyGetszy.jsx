...
export default function MyGetszy() {
  ...
  const [state, setState] = useState({ loading: true, orders: [], enrollments: [], subscription: null, sessions: [], builderProjects: [], credits: null, creditCosts: {}, creditTransactions: [], creditHistoryAvailable: false });
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      ...
      setState({
        ...
        creditHistoryAvailable: creditTransactions.status === "fulfilled",
      });
    });
    return () => { active = false; };
  }, []);
        ...
        <>
          <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4" aria-label="Your Getszy overview">
            ...
            <button type="button" onClick={() => navigate("/dashboard")} className="rounded-2xl border bg-white p-5 text-left transition-colors hover:bg-[#fcfcfb]" style={{ borderColor: "var(--gs-border)" }}>
              ...
              <div className="mt-1 text-sm text-[var(--gs-muted)]">{state.builderProjects.length ? "Professional project(s) in your workspace" : "Start a mission with Reva"}</div>
            </button>
            ...
            <button type="button" onClick={() => navigate("/account")} className="rounded-2xl border bg-white p-5 text-left transition-colors hover:bg-[#fcfcfb]" style={{ borderColor: "var(--gs-border)" }}>
              ...
              <div className="mt-5 text-xs font-semibold uppercase tracking-[.14em] text-[var(--gs-muted)]">Credit account & profile</div>
              <div className="mt-1 font-display text-2xl text-[var(--gs-ink)]">Prepaid</div>
              <div className="mt-1 text-sm text-[var(--gs-muted)]">{state.credits === null ? "Manage account settings" : `${state.credits} credits available`}</div>
            </button>
          </section>
          ...
          <section className="grid gap-4 xl:grid-cols-[1.25fr_.95fr]">
            ...
            <div className="rounded-3xl border bg-[#fffdf9] p-5 sm:p-6" style={{ borderColor: "var(--gs-border)" }}>
              <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[.14em] text-[var(--gs-teal)]"><CreditCard className="h-4 w-4" /> Your Getszy account</div>
              <h2 className="mt-2 font-display text-2xl text-[var(--gs-ink)]">Credits, learning and profile.</h2>
              <div className="mt-5 space-y-3">
                <div className="rounded-2xl border bg-white p-4" style={{ borderColor: "var(--gs-border)" }}>
                  <div className="text-sm font-semibold text-[var(--gs-ink)]">Your prepaid credit account</div>
                  <p className="mt-1 text-sm text-[var(--gs-muted)]">Digital work is available only when your paid credit balance covers the action cost. Credits are not a monthly-access allowance.</p>
                  ...
                  <div className="mt-3 flex flex-wrap gap-3"><Link to="/pricing" className="inline-flex items-center gap-1 text-sm font-semibold text-[var(--gs-teal)] hover:underline">Choose a credit pack <ArrowRight className="h-4 w-4" /></Link><Link to="/account" className="inline-flex items-center gap-1 text-sm font-semibold text-[var(--gs-teal)] hover:underline">Manage profile <ArrowRight className="h-4 w-4" /></Link></div>
                </div>
                <div className="rounded-2xl border bg-white p-4" style={{ borderColor: "var(--gs-border)" }}>
          ...
          <section className="grid gap-4 xl:grid-cols-[1.1fr_.9fr]">
            <div className="rounded-3xl border bg-white p-5 sm:p-6" style={{ borderColor: "var(--gs-border)" }}>
              ...
              <div className="mt-5 space-y-2">
                ...
                {activeBuilderProjects.length === 0 && activeProjectSessions.length === 0 && <EmptyState title="No digital mission has started" detail="Start with Reva when you are ready. Getszy will show what it understands before you approve a plan." action="Talk to Reva" onClick={() => navigate("/dashboard")} />}
                {activeBuilderProjects.length === 0 && activeProjectSessions.map((session) => (
                  <Link key={session.session_id} to={`/dashboard/chat/${session.session_id}`} className="flex items-center gap-3 rounded-2xl border p-4 transition-colors hover:bg-[#fbfdfc]" style={{ borderColor: "var(--gs-border)" }}>
                    ...
                    <div className="min-w-0 flex-1"><div className="font-semibold text-sm text-[var(--gs-ink)]">{session.agent_name || "Reva conversation"}</div><div className="mt-1 truncate text-xs text-[var(--gs-muted)]">{session.last_message || "Continue your guided conversation"}</div></div>
                    <ChevronRight className="h-4 w-4 text-[var(--gs-muted)]" />
                  </Link>
                ))}
              </div>
