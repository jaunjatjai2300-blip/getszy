          ...
          <div className="mt-5 overflow-hidden rounded-2xl border" style={{ borderColor: "var(--gs-border)", background: "var(--gs-surface-2)" }}>
            <div className="flex flex-col gap-3 border-b px-4 py-3 sm:flex-row sm:items-center sm:justify-between" style={{ borderColor: "var(--gs-border)", background: "white" }}>
              ...
              <div className="flex flex-wrap items-center gap-1.5">
                ...
                {previewUrl ? <a href={previewUrl} target="_blank" rel="noreferrer" className="inline-flex h-8 items-center gap-1 rounded-md border px-2 text-xs font-semibold text-[var(--gs-teal)] hover:bg-[var(--gs-surface-2)]" style={{ borderColor: "var(--gs-border)" }}><ArrowRight className="h-3.5 w-3.5" />Open</a> : previewTokenError ? <span className="inline-flex h-8 items-center text-xs font-semibold text-rose-600">Preview link unavailable</span> : null}
              </div>
            </div>
            <div className="overflow-auto p-4"><div className={`mx-auto overflow-hidden rounded-xl border bg-white ${previewDevice === "mobile" ? "h-[680px] max-w-[390px]" : previewDevice === "tablet" ? "h-[620px] max-w-[768px]" : "h-[560px] w-full"}`} style={{ borderColor: "var(--gs-border)" }}>{previewUrl ? <iframe src={previewUrl} className="h-full w-full" title={`Private preview of ${project.name}`} data-testid="customer-project-preview" /> : previewTokenError ? <div className="flex h-full items-center justify-center p-6 text-center text-sm text-[var(--gs-muted)]">The secure preview link could not be prepared. Reload the project to retry.</div> : <div className="flex h-full items-center justify-center gap-2 text-sm text-[var(--gs-muted)]"><Loader2 className="h-4 w-4 animate-spin" />Preparing secure preview…</div>}</div></div>
